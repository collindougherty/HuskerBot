import tweepy
# import credentials.py from /Users/collindougherty/Documents/Credentials/credentials.py
import sys
sys.path.append('/Users/collindougherty/Documents/Credentials')
...
def retweet_bunch(api, count):
    for handle in retweets.RT_HANDLES:
        tweets = api.user_timeline(screen_name=handle, count = count, exclude_replies = True)
        for tweet in tweets:
            if TWEET_COUNT < retweets.MAX_TWEETS:
                rt(api, tweet.id)
            else:
